...
function embedme(sourceText, inputFilePath, options) {
    ...
    while ((result = codeFenceFinder.exec(sourceText)) !== null) {
        ...
        const { text, error } = getReplacement(inputFilePath, options, log, codeFence, leadingSpaces, lineEnding, codeExtension, firstLine || '', startLineNumber, /<!--\s*?embedme[ -]ignore-next\s*?-->/g.test(start), commentInsertion ? commentInsertion[1] : undefined);
        if (error) {
